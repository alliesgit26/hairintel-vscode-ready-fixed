...
function renderS16Export(params = {}) {
  ...
  const aiCard = aiLimit === 0 ? `
    <div class="hi-card" style="border-color:var(--gold-border);background:var(--gold-pale);">
      ...
      <p style="font-size:13px;color:var(--text-sub);margin-bottom:12px;">AI hair preview is available on Pro and Studio plans.</p>
    ...
    <div class="hi-card" style="border-color:var(--gold-border);background:var(--bg-card);">
      ...
      <p style="font-size:13px;color:var(--text-sub);margin-bottom:12px;">Each generation uses 1 monthly AI preview credit.</p>
      <button class="hi-btn hi-btn-gold" ${!canAI ? 'disabled style="opacity:.6;cursor:not-allowed;"' : ''} onclick="HIApp.go('ai-preview', { consultId: '${consultId}' })">
  ...
  <div class="hi-screen hi-animate-fade" id="screen-export">
    ...
    <div class="hi-content">
      ...
      <div class="hi-disclaimer hi-mb-4">
        <strong style="color:var(--warning);">Safety Disclaimer:</strong> HairIntel AI is a decision-support tool for licensed or professional hairstylists. It supports consultation planning but does not replace professional in-person judgment.
      </div>

      <div style="display:flex;flex-direction:column;gap:10px;">
        ${isPro ? `
        <button class="hi-btn hi-btn-gold" id="export-pdf-btn">${HIcons.download} &nbsp;Export PDF Report</button>
        <button class="hi-btn hi-btn-outline" id="export-share-btn">${HIcons.share} &nbsp;Share with Client</button>
        ` : `
        <div class="hi-card" style="border-color:var(--gold-border);background:var(--gold-pale);">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
          ...
          <p style="font-size:13px;color:var(--text-sub);margin-bottom:12px;">PDF export and client sharing require a Pro or Studio subscription.</p>
          <button class="hi-btn hi-btn-gold" onclick="HIApp.go('subscription')">Upgrade to Pro</button>
        </div>`}

        ${aiCard}

        <button class="hi-btn hi-btn-outline" id="save-consult-btn">${HIcons.check} &nbsp;Save to Client Profile</button>
        <button class="hi-btn hi-btn-ghost" onclick="HIApp.go('clients')">${HIcons.clients} &nbsp;View All Clients</button>
        <button class="hi-btn hi-btn-ghost" onclick="HIApp.go('welcome')">Return to Dashboard</button>
      </div>
    </div>
  </div>`;
}
...
function initS16Export(params = {}) {
  ...
  if (isPro) {
    hEl('export-pdf-btn')?.addEventListener('click', () => {
      ...
      setTimeout(() => hiToast('PDF ready — tap to open (simulated)','success'), 1800);
    });

    hEl('export-share-btn')?.addEventListener('click', () => {
      const consultRef = consultId ? `ConsultID: ${consultId}` : 'HairIntel AI Consultation';
      if (navigator.share) {
        navigator.share({ title: 'HairIntel AI Consultation', text: `Your HairIntel AI consultation summary is ready. ${consultRef}`, url: window.location.href }).catch(() => hiToast('Share cancelled','info'));
      } else {
