(function () {
  ...
  function findAiPreview(consult) {
    ...
    const consultId = consult?.id || consult?.consultId || null;
    if (!cached.consultId || !consultId || cached.consultId === consultId) return cached.image;
    return null;
  }
  ...
  function ensurePlacementStyles() {
    ...
    style.textContent = `
      .head-frame.hi-ai-placement-frame{
        ...
        aspect-ratio:4/5!important;min-height:340px!important;box-shadow:0 20px 42px rgba(55,31,41,.12)!important
      }
      .head-frame.hi-ai-placement-frame>img{
        display:block!important;width:100%!important;height:100%!important;object-fit:cover!important;object-position:center top!important;
      ...
      .hi-placement-overlay svg{display:block;width:100%;height:100%;overflow:visible}
      .hi-placement-line{fill:none;stroke:#e9bd72;stroke-width:1.7;stroke-linecap:round;stroke-dasharray:2.2 5;filter:drop-shadow(0 1px 3px rgba(28,13,19,.72))}
      .hi-placement-line.caution{stroke:#d59183;stroke-dasharray:6 4}
      .hi-placement-line.avoid{stroke:#f1d1c2;stroke-dasharray:1 7;opacity:.78}
      .hi-placement-dot{fill:#efc579;stroke:rgba(54,28,39,.82);stroke-width:.65;filter:drop-shadow(0 1px 2px rgba(0,0,0,.7))}
      .hi-placement-dot.caution{fill:#d99686}
      .hi-placement-badge{
        position:absolute;z-index:4;top:12px;left:12px;display:inline-flex;align-items:center;gap:7px;
        padding:7px 10px;border-radius:999px;background:rgba(38,20,30,.78);color:#fff7eb;
        border:1px solid rgba(235,196,126,.45);backdrop-filter:blur(10px);font:700 9px/1 Inter,sans-serif;
        text-transform:uppercase;letter-spacing:.11em;box-shadow:0 8px 24px rgba(0,0,0,.2)
      }
      .hi-placement-badge:before{content:"✦";color:#edc67e;font-size:10px}
      .hi-placement-overlay[data-view="side"]{transform:translateX(4%) scale(.97);transform-origin:center}
      .hi-placement-overlay[data-view="back"]{transform:scaleX(.96);transform-origin:center}
      .client-card .client-avatar.hi-client-ai-avatar{overflow:hidden!important;background:#21161b!important}
      .client-card .client-avatar.hi-client-ai-avatar img{width:100%!important;height:100%!important;object-fit:cover!important;object-position:center top!important}
      @media(max-width:870px){
        .head-frame.hi-ai-placement-frame{min-height:390px!important;width:100%!important;max-width:100%!important;margin:0 auto!important}
      }
      @media(max-width:560px){
        .head-frame.hi-ai-placement-frame{min-height:360px!important;border-radius:12px!important}
  ...
  function placementSvg(map = {}) {
    ...
    return `
      <svg viewBox="0 0 100 125" preserveAspectRatio="none" aria-hidden="true">
        ...
        <circle class="hi-placement-dot ${cautionCrown ? 'caution' : ''}" cx="31" cy="28" r="1.35"/>
        <circle class="hi-placement-dot ${cautionCrown ? 'caution' : ''}" cx="42" cy="25.2" r="1.35"/>
        <circle class="hi-placement-dot ${cautionCrown ? 'caution' : ''}" cx="54" cy="25.2" r="1.35"/>
        <circle class="hi-placement-dot ${cautionCrown ? 'caution' : ''}" cx="68" cy="28" r="1.35"/>
        ...
        <circle class="hi-placement-dot ${avoidNape ? 'caution' : ''}" cx="28" cy="72.5" r="1.35"/>
        <circle class="hi-placement-dot ${avoidNape ? 'caution' : ''}" cx="42" cy="69.8" r="1.35"/>
        <circle class="hi-placement-dot ${avoidNape ? 'caution' : ''}" cx="58" cy="69.8" r="1.35"/>
        <circle class="hi-placement-dot ${avoidNape ? 'caution' : ''}" cx="72" cy="72.5" r="1.35"/>
      </svg>`;
  }
  ...
  function updateScorePanelLanguage(result, capacity) {
    ...
    if (denom) denom.textContent = "/100";
  ...
  function hydrateDashboard() {
    ...
    updateScorePanelLanguage(result, capacity);
    setPlacement("top");
