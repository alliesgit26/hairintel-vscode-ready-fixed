      ...
      <div class="hi-card hi-mb-4" style="background:var(--gold-pale);border-color:var(--gold-border);">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;">
          <div>
            ...
            <div style="font-size:12px;color:var(--text-muted);">${plan.rows} rows Â· ${plan.grams}g Â· ${plan.maintenance} maintenance</div>
          </div>
      ...
      <div class="hi-card hi-mb-4">
        ...
        <div style="display:flex;flex-direction:column;gap:10px;">
          ...
          ${[
            ...
            { title:'Time',           body:'Tape-in appointments take 2â€“3 hours. Hand-tied wefts can require 4â€“6 hours in the chair.' }
          ].map(tip => `
      ...
      <div id="export-preview-card" class="hi-export-card">
        ...
        <div class="hi-export-section" style="border-top:1px solid var(--border);padding-top:12px;margin-top:0;">
          <p style="font-size:10px;color:var(--text-muted);line-height:1.5;text-align:center;">
            Generated by HairIntel AI Â· ${stylist.name || 'Licensed Stylist'} Â· ${hiDate(new Date().toISOString())}
            ${!isPro ? '<br><strong>Free Plan â€” Watermark Applied</strong>' : ''}
          </p>
        </div>

        <!-- Free plan watermark -->
        ${!isPro ? `<div class="hi-export-watermark">HairIntel AI Â· Free</div>` : ''}
      </div>
      ...
      <div style="display:flex;flex-direction:column;gap:10px;">
        ...
        <div class="hi-card" style="border-color:var(--gold-border);background:var(--gold-pale);">
          <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
            ...
            <div class="hi-label" style="color:var(--gold);">AI Hair Preview</div>
          </div>
          <p style="font-size:13px;color:var(--text-sub);margin-bottom:12px;">Generate a photorealistic extension preview using the client photos and recommended install plan.</p>
          <button class="hi-btn hi-btn-gold" onclick="HIApp.go('ai-preview', { consultId: '${consultId}' })">Open AI Hair Preview</button>
        </div>
        ${isPro ? `
...
function initS16Export(params = {}) {
  ...
  if (isPro) {
    hEl('export-pdf-btn')?.addEventListener('click', () => {
      ...
      setTimeout(() => {
        hiToast('PDF ready â€” tap to open (simulated)','success');
      }, 1800);
    });
